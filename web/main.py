    ...
    try:
        ...
        if not os.path.exists(absolute_save_folder):
            try:
                ...
                raise ValueError(f"无法创建保存文件夹: {absolute_save_folder}, 错误: {str(e)}")
        ...
        if not use_sam_bool:
            ...
            # 保存图像文件名
            image_filename = os.path.splitext(os.path.basename(video_file.filename))[0] + ".jpg"
            image_path = os.path.join(output_dir, image_filename)
            
            # 保存视频文件名
            video_filename = os.path.splitext(os.path.basename(video_file.filename))[0] + ".mp4"
            ...
            # 使用SAM进行处理
            # 验证SAM相关参数
            if sam_checkpoint is None or sam_model_type is None or sam_model_device is None:
            ...
            # 创建数据集
            dataset_maker = QuickMakeDataSet(
